...
let lastAdded = new Cat(defaultData.Cat);
let lastAddedDog = new Dog(defaultData.Dog);
...
const updateDogAgeByName = (req, res) => {
  if (!req.body.name) {
    return res.status(400).json({ error: 'Name is required to perform a search' });
  }

  let entryDog = new Dog(defaultData.Dog);
  ...
  const callback = (err, doc) => {
    ...
    entryDog = new Dog(doc);
    entryDog.age++;

    return entryDog.save();
  };

  return Dog.findByName(req.body.name, (err, doc) => {
    const savePromise = callback(err, doc);
    savePromise.then(() => res.json({
      name: entryDog.name,
      breed: entryDog.breed,
      age: entryDog.age,
...
module.exports = {
  ...
  setCatName: setName,
  setDogName,
  updateLastCat: updateLast,
  updateLastDog,
  updateDogAgeByName,
  searchCatName: searchName,
  searchDogName,
  notFound,
};
